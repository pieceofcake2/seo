...
{
    public $uses = ['Seo.SeoUrl', 'Seo.SeoUri', 'Seo.SeoRedirect'];
    ...
    public function search()
    {
        ...
        $urls = $this->SeoUri->find('all', [
            'contain' => ['SeoRedirect'],
            'conditions' => ['SeoUri.uri LIKE' => $term . '%'],
            ]);
        if (empty($urls)) {
            $urls = $this->SeoUri->find('all', [
                'contain' => ['SeoRedirect'],
        ...
        $redirects = $this->SeoRedirect->find('all', [
            'contain' => ['SeoUri'],
            'conditions' => ['SeoRedirect.redirect LIKE' => $term . '%'],
            ]);
        if (empty($redirects)) {
            $redirects = $this->SeoRedirect->find('all', [
                'contain' => ['SeoUri'],
    ...
    public function add()
    {
        ...
        if (empty($url) || strlen($url) < 3) {
            $this->errorAndExit("Sorry, bad/missing input <url> = '$url'");
        }
        if (!in_array(substr($url, 0, 1), ['/', '#'])) {
            $this->errorAndExit("Sorry, the input <url> should start with a '/' or a '#' you put in: '$url'");
        }
        if (empty($redirect) || (strlen($redirect) < 3 && substr($url, 0, 1) != '/')) {
            $this->errorAndExit("Sorry, bad/missing input <redirect> = '$redirect'");
        }
        if (substr($redirect, 0, 1) !== '/' && substr($redirect, 0, 5) !== 'http' && strpos($redirect, '{callback}') === false) {
            $this->errorAndExit("Sorry, the input <redirect> should start with a '/' or a 'http' you put inredirecturl'");
        }

        $save = [
            'SeoUri' => ['uri' => $url, 'is_approved' => 1],
            'SeoRedirect' => compact('redirect', 'priority', 'callback'),
        ];
        $existing = $this->SeoUri->find('first', [
            'contain' => ['SeoRedirect'],
            'conditions' => ['SeoUri.uri LIKE' => $url . '%'],
        ]);
        if (!empty($existing) && isset($existing['SeoRedirect']['id']) && !empty($existing['SeoRedirect']['id'])) {
            ...
            $this->errorAndExit("Want to change it?  you're going to have to do so via the web interface.");
        }
        $this->SeoRedirect->clear();
        if ($this->SeoRedirect->save($save)) {
            $redirect = $this->SeoRedirect->find('first', [
                'contain' => ['SeoUri'],
                'conditions' => ['SeoRedirect.id' => $this->SeoRedirect->id],
                ]);
            $this->out('Saved.');
            $this->out("    {$redirect['SeoUri']['uri']} --> {$redirect['SeoRedirect']['redirect']}");
            $this->out("        Uri #{$redirect['SeoUri']['id']} --> redirect #{$redirect['SeoRedirect']['id']}");
    ...
    public function delete(): void
    {
        while (!empty($this->args)) {
            ...
            if (empty($seo_redirect_id)) {
                $this->errorAndExit("Missing or empty SEO Redirect ID -- can not delete it: $seo_redirect_id");
            }
    ...
    public function delete_uri()
    {
        while (!empty($this->args)) {
            ...
            if (empty($id)) {
                $this->errorAndExit("Missing or empty SEO URI ID -- can not delete it: $id");
            }
