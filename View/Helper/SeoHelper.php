...
class SeoHelper extends AppHelper
{
     ...
     */
    public function metaTags($metaData = [])
    {
        ...
        if (!empty($metaData)) {
            foreach ($metaData as $name => $content) {
                $retval .= $this->Html->meta(['name' => $name, 'content' => $content]);
            }
        }
     ...
     * @return html|null tags with dns-prefetch
     */
    public function dnsPrefetch($prefetches = [])
    {
        ...
        foreach ($prefetches as $dns) {
            $retval .= $this->Html->tag('link', null, ['rel' => 'dns-prefetch', 'href' => $dns]);
        }
     ...
     */
    public function canonical($url = null, $full = true, $overwrite = false)
    {
        ...
        if ($url) {
            ...
            return $this->Html->tag('link', null, ['rel' => 'canonical', 'href' => $path]);
        }
     ...
     */
    public function honeyPot($title = 'Click Here', $options = [])
    {
        $options = array_merge(
            [
                'rel' => 'nofollow',
     ...
     *
     * @param mixed $test test to show code for (if null, will check the View for ABTest variable and use that.
     ...
     * @return string|null ga script test, or null
     */
    public function getABTestJS($test = null, array $options = [])
    {
        if (!$test) {
